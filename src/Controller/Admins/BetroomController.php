...
class BetroomController extends AbstractController
{
    ...
    #[Route('/admin/drawing-rooms', name: 'admins.betrooms')]
    public function Betroom(Request $request): Response
    {
        ...
        $roomSettings = !empty($settings) ? $settings[0] : null;
    ...
    #[Route('/admin/drawing-rooms/{reference}', name: 'admins.betrooms.edit')]
    public function editBetroom(Request $request, Betroom $betroom): Response
    {
        ...
        $tickets = $betroom->getTickets();
        $old_max_ticket = !empty($tickets) ? $tickets[0]->getBetroom()->getMaxTicket() : $betroom->getMaxTicket();
        $form = $this->createForm(AddBetroomType::class, $betroom);
        $form->handleRequest($request);
        if ($form->isSubmitted() && $form->isValid()) {
    ...
    #[Route('/admin/drawing-rooms/deletion/{reference}', name: 'admins.betrooms.remove')]
    ...
    #[Route('/admin/daily-results', name: 'admins.winners')]
