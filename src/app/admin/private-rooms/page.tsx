...
import { 
  Users, Search, Trash2, Edit2, 
  DollarSign, Calendar, RefreshCw, User, Trophy
} from "lucide-react";
...
export default function PrivateRoomManagement() {
  ...
  const loadRooms = async () => {
    setLoading(true);
    const res = await getAllLotteriesAction(true); // isPrivate: true
  ...
  const handleDelete = async (id: string) => {
    if (!confirm("Are you sure? This will delete the user's private room. If bets exist, this will fail.")) return;
    const res = await deleteLotteryAction(id);
    if (res.success) {
       toast.success("Private room removed");
       loadRooms();
    } else {
    ...
    <div className="flex flex-col gap-10">
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-6 pb-6 border-b border-black/5">
        <div className="flex flex-col gap-2">
          ...
          <p className="font-bold text-text-muted">Monitor and moderate private lottery rooms created by platform users.</p>
        </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-6">
        {loading ? (
             <div className="col-span-full py-20 flex flex-col items-center gap-4">
                 ...
                 <span className="font-black text-[10px] uppercase tracking-[4px] text-text-muted">Scanning Community...</span>
             </div>
           ...
           >
              <div className="flex items-center justify-between relative z-10">
                 <div className="w-14 h-14 rounded-2xl bg-zinc-50 flex items-center justify-center text-zinc-400 shadow-md group-hover:bg-primary-gold/10 group-hover:text-primary-gold transition-colors">
              ...
              <div className="flex flex-col gap-2 relative z-10 p-2">
                 ...
                 <div className="flex items-center gap-2 text-[10px] font-black text-text-muted uppercase tracking-widest">
                    <span className="text-primary-gold">Created by:</span> {room.creator?.name || 'Unknown User'}
                 </div>
              </div>

              <div className="grid grid-cols-2 gap-4 relative z-10">
                 <div className="p-4 bg-zinc-50 rounded-2xl flex flex-col gap-1 border border-black/5">
                    <span className="text-[9px] font-black text-text-muted uppercase tracking-widest leading-none mb-1">Room Pot</span>
                 ...
                 <div className="p-4 bg-zinc-50 rounded-2xl flex flex-col gap-1 border border-black/5">
                    <span className="text-[9px] font-black text-text-muted uppercase tracking-widest leading-none mb-1">Participants</span>
